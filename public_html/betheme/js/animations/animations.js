...
function animateOnce(a, n) {
	'random' == n && (n = randomClass()),
		jQuery(a)
			.removeClass('trigger infinite ' + triggerClasses)
			.addClass('trigger')
			.addClass(n)
			.one(
	...
	jQuery(window).on('scroll', function () {
		animateElement(),
			jQuery(window).scrollTop() + jQuery(window).height() ==
				jQuery(document).height() && animateElement();
	});
var triggerClasses =
		'flash strobe shakeH shakeV bounce tada wave spinCW spinCCW slingshotCW slingshotCCW wobble pulse pulsate heartbeat panic',
