		...
		for (var i in K) {
			if (typeof E[i] !== 'function') {
				...
				K[i].rightEdge.css({
					...
					background: j[i],
					transform:
						'rotateY(90deg) translate3d(' + H / 2 + 'px,0,-' + (I - H / 2) + 'px)',
				});
				K[i].leftEdge.css({
					width: H,
					height: J,
		...
		if (p) {
			...
			for (var L in E) {
				...
				E[L].part
					...
					.animate({ right: 0, opacity: 1 }, W, function () {
						H++;
						if (H == a) {
							s.stop(1, 1);
							d = false;
