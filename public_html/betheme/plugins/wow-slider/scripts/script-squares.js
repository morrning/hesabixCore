...
jQuery.fn.coinslider ||
	(function (t) {
		var e = new Array(),
			...
			h = new Array(),
			u = new Array();
		(t.fn.coinslider = t.fn.CoinSlider =
			function (l) {
				(init = function (o) {
					for (
						a[o.id] = new Array(),
							...
							t.each(t('#' + o.id + ' img'), function (i, e) {
								(n[o.id][i] = e),
									(s[o.id][i] = t(e).parent().is('a')
										? t(e).parent().attr('href')
										: ''),
									(d[o.id][i] = t(e).parent().is('a')
										? t(e).parent().attr('target')
										: ''),
									(r[o.id][i] = t(e).next().is('span')
										? t(e).next().html()
										: '');
							}),
							u[o.id] = t(
								"<div class='coin-slider' id='coin-slider-" + o.id + "' />"
							)
								.css({
									'background-image':
										'url(' + n[o.id][l.startSlide || 0].src + ')',
						...
						for (j = 1; j <= e[o.id].spw; j++)
							t(
								e[o.id].links
									? "<a href='../../wow-slider/engine1/" + s[o.id][0] + "'></a>"
									: '<div></div>'
							)
								.css({
									float: 'right',
					...
					u[o.id].append(
						"<div class='cs-title' id='cs-title-" +
							o.id +
							"' style='position: absolute; bottom:0; right: 0; z-index: 1000;'></div>"
					),
					...
					(t.setFields = function (s) {
						var d = u[s.id].width(),
							r = u[s.id].height();
						for (
							tWidth = sWidth = parseInt(d / e[s.id].spw),
								...
								imgParams = {
									...
									marginRight: parseFloat(imgParams.css('marginRight')),
								},
								i = 1;
							i <= e[s.id].sph;
							i++
							...
							for (
								...
								gapx > 0 ? (gapx--, (sWidth = tWidth + 1)) : (sWidth = tWidth),
									(a[s.id][counter] = i + '' + j),
									counter++,
									t('#cs-' + s.id + i + j)
										.css({
										...
										.css({
											...
											marginRight: imgParams.marginRight - sLeft,
										}),
									(sLeft += sWidth);
					...
					(t.transitionCall = function (i) {
						e[i.id].delay < 0 ||
							(clearInterval(o[i.id]),
							(delay =
								e[i.id].delay + e[i.id].spw * e[i.id].sph * e[i.id].sDelay),
					...
					(t.transition = function (i, s) {
						1 != e[i.id].pause &&
							...
							t('#cs-button-' + i.id + '-' + (c[i.id] + 1)).addClass(
								'cs-active'
							),
							r[i.id][c[i.id]]
								? (t('#cs-title-' + i.id)
										.css({ opacity: 0 })
										.animate({ opacity: e[i.id].opacity }, e[i.id].titleSpeed),
								  t('#cs-title-' + i.id).html(r[i.id][c[i.id]]))
								: t('#cs-title-' + i.id).css('opacity', 0));
					}),
					(t.appereance = function (i, a) {
						return (
							t('.cs-' + i.id)
								.attr('href', s[i.id][c[i.id]])
								.attr('target', d[i.id][c[i.id]]),
							p[i.id] == e[i.id].spw * e[i.id].sph
								? (clearInterval(f[i.id]),
					...
					(t.setNavigation = function (i) {
						for (
							t(u[i.id]).append("<div id='cs-navigation-" + i.id + "'></div>"),
								t('#cs-navigation-' + i.id).hide(),
								t('#cs-navigation-' + i.id).append(
									"<a href='#' id='cs-prev-" +
										i.id +
								...
								t(
									"<div id='cs-buttons-" + i.id + "' class='cs-buttons'></div>"
								).appendTo(t('#coin-slider-' + i.id)),
								k = 1;
							k < n[i.id].length + 1;
							k++
						)
							t('#cs-buttons-' + i.id).append(
						...
						t.each(t('.cs-button-' + i.id), function (e, a) {
							...
							t('#cs-buttons-' + i.id).css({
								right: '50%',
								'margin-right': (15 * -n[i.id].length) / 2 - 5,
								position: 'relative',
							});
					}),
					(t.effect = function (n) {
						if (
							...
							(eff =
								'' == e[n.id].effect
									? effA[Math.floor(Math.random() * effA.length)]
									: e[n.id].effect),
							(a[n.id] = new Array()),
							'random' == eff)
						) {
...
function ws_squares(i, h, b) {
	...
	var d = e('#coin-slider-' + f.id)
		.addClass('ws_effect ws_squares')
		.css({
			position: 'absolute',
			display: 'none',
			right: 0,
