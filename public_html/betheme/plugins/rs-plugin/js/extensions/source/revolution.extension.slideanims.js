		...
		jQuery.each(
			...
			function (i, b) {
				if (comingtransition == b + 'horizontal')
					comingtransition = slidedirection != 1 ? b + 'right' : b + 'left';
...
*************************************/
	...
	var animateSlideIntern = function (
		...
		if (nexttrans == 0) {
			...
			// ALL NEW SLOTS SHOULD BE SLIDED FROM THE RIGHT TO THE LEFT
		...
		if (nexttrans == 2) {
			...
			// ALL OLD SLOTS SHOULD BE SLIDED TO THE LEFT
			...
			nextsh.find('.slotslide').each(function (i) {
				...
				subtl.add(
					punchgs.TweenLite.fromTo(
						ss,
						masterspeed / 1000,
						{ right: 0, top: 0, opacity: 0, transformPerspective: 600 },
						{
							right: 0 - i * opt.slotw + 'px',
							ease: eo,
							force3D: 'auto',
							top: 0 + 'px',
							width: opt.width,
			...
			nextsh.find('.slotslide').each(function (i) {
				...
				subtl.add(
					punchgs.TweenLite.fromTo(
						ss,
						masterspeed / 1000,
						{ right: 0, top: 0, opacity: 0, force3D: 'auto' },
						{
							right: 0 + 'px',
		...
		if (
			...
			slot.append(
				'<span style="background-color:rgba(0,0,0,0.6);width:100%;height:100%;position:absolute;top:0px;right:0px;display:block;z-index:2"></span>'
			);
				...
				cont.find('.slot-circle-wrapper').each(function (i) {
					if (i < pieces) {
						var t = jQuery(this),
							...
							tb = { top: nt + 'px', right: nl + 'px', ease: ei },
							speed = ms,
							delay = 0;
		...
		if (nexttrans == 16) {
			...
			actli
				...
				.wrapAll('<div style="position:absolute;top:-50%;right:0px;"></div>');
