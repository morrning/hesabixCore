		...
		animateSingleCaption: function (
			...
			if (opt.debugMode) {
				_nc
					.closest('li')
					.find('.helpgrid')
					.css({ top: offsety + 'px', right: offsetx + 'px' });
			...
			_nc.append('<div class="tp-videoposter" style="position:absolute;top:0px;right:0px;width:100%;height:100%;background-image:url('+_nc.data('videoposter')+'); background-size:cover;background-position:center center;"></div>');
			if (vidw!="100%")
			...
			//arrobj.hoa = arrobj.elx==="left" ? arrobj.hoa * -1 : arrobj.hoa;
			...
			// SET TOP/RIGHT POSITION OF LAYER
...
**********************************************************************************************/
	...
	var getBorderDirections = function (x, o, w, h, top, right, direction) {
		if (!jQuery.isNumeric(x) && x.match(/%]/g)) {
			...
			x = x === 'layer_right' ? 0 - w : x === 'layer_left' ? w : x;
			x = x === 'layer_top' ? 0 - h : x === 'layer_bottom' ? h : x;
			x =
				x === 'right' || x === 'stage_left'
					? 0 - w - right
					: x === 'left' || x === 'stage_left'
					? o.conw - right
