...
(function ($) {
	...
	jQuery.extend(true, _R, {
				...
				if (ckNO(otbu) && force) {
					...
					bw.find('.tp-bullet').each(function (i) {
						...
						if (otbu.direction === 'vertical') {
							b.css({ top: (am - 1) * h + 'px', right: '0px' });
							bw.css({
								height: (am - 1) * h + b.outerHeight(),
								width: b.outerWidth(),
							});
						} else {
							b.css({ right: (am - 1) * w + 'px', top: '0px' });
							bw.css({
		...
		// PUT NAVIGATION IN POSITION AND MAKE SURE THUMBS AND TABS SHOWING TO THE LEFT POSITION
		manageNavigation: function (opt) {
			...
			if (ckNO(opt.navigation.bullets)) {
				if (
					...
					// OFFSET ADJUSTEMENT FOR RIGHT ARROWS BASED ON THUMBNAILS AND TABS OUTTER
				...
				if (
					...
					opt.navigation.arrows.right.h_offset_old =
						opt.navigation.arrows.right.h_offset_old === undefined
							...
							: opt.navigation.arrows.right.h_offset_old;
					opt.navigation.arrows.right.h_offset =
						opt.navigation.arrows.right.h_align === 'left'
					...
					opt.navigation.arrows.left.h_offset_old =
						opt.navigation.arrows.left.h_offset_old === undefined
							...
							: opt.navigation.arrows.left.h_offset_old;
					opt.navigation.arrows.left.h_offset =
						opt.navigation.arrows.left.h_align === 'left'
		...
		if (fin)
			jQuery.each(fin, function (i, el) {
				if (o.direction === 'vertical')
					...
					tws.add(
						punchgs.TweenLite.set(el, {
							right: i * (cw + parseInt(o.space === undefined ? 0 : o.space, 0)),
							width: cw + 'px',
							height: ch + 'px',
						})
					);
		...
		SwipeOn.swipetp({
			...
			swipeStatus: function (
				...
				if (
					...
					distance =
						direction && direction.match(/right|up/g)
				...
				// SWIPE OVER THUMBS OR TABS
					...
					switch (phase) {
						...
						case 'move':
							...
							else punchgs.TweenLite.set(el, { right: curpos + 'px' });
	...
	// PUT ELEMENTS VERTICAL / HORIZONTAL IN THE LEFT POSITION
	var putVinPosition = function (el, o) {
		var elh = el.outerHeight(true),
			elw = el.outerWidth(true),
			a =
				o.v_align === 'top'
					? { top: '0px', y: Math.round(o.v_offset) + 'px' }
					: o.v_align === 'center'
	...
	var putHinPosition = function (el, o) {
		var elh = el.outerHeight(true),
			...
			a =
				o.h_align === 'right'
					? { right: '0px', x: Math.round(o.h_offset) + 'px' }
					: o.h_align === 'center'
					? { right: '50%', x: Math.round(0 + elw / 2 + o.h_offset) + 'px' }
					: { right: '100%', x: Math.round(0 + (elw + o.h_offset)) + 'px' };
