...
(function ($) {
	...
	jQuery.extend(true, _R, {
		...
		carouselToEvalPosition: function (opt, direction) {
			...
			calc =
				(cm >= 0.3 && direction === 'right') ||
				(cm >= 0.7 && direction === 'left')
					? mc
					: (cm < 0.3 && direction === 'right') ||
					  (cm < 0.7 && direction === 'left')
					? mf
		...
		organiseCarousel: function (opt, direction, setmaind, unli) {
			...
			var _ = opt.carousel,
				...
				len = _.slides.length,
				leftlimit =
					_.horizontal_align === 'left' ? (leftlimit = opt.width) : 0;
				...
				if (_.infinity === 'on') {
					pos =
						pos > _.wrapwidth - _.inneroffset && direction == 'left'
							...
							: pos;
					pos =
						pos < 0 - _.inneroffset - _.slide_width && direction == 'right'
							? pos + _.maxwidth
							: pos;
				}
...
***************************************************/
	...
	var setCarouselDefaults = function (opt) {
		...
		_.inneroffset =
			_.horizontal_align === 'left' ? _.wrapwidth - _.slide_width : 0;
