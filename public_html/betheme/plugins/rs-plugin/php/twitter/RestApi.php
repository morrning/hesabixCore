...
class RestApi
{
    ...
    public function __construct($consumerKey, $consumerSecret, $accessToken = null, $accessTokenSecret = null)
    {
        if (!function_exists('curl_init')) {
            throw new TwitterRestApiException('برای استفاده از این کتابخانه باید پسوند cURL را فعال کنید');
        }
    ...
    public function connectAsUser()
    {
        if (!$this->_accessToken || !$this->_accessTokenSecret) {
            throw new TwitterRestApiException('ACCESS_TOKEN یا ACCESS_TOKEN_SECRET وجود ندارد');
        }
