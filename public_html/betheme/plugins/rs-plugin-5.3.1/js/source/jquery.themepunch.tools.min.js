...
(function (factory) {
	...
	*/
	...
	function TouchSwipe(element, options) {
		...
		function touchMove(jqEvent) {
			...
			if (
				...
				if (!options.triggerOnTouchEnd || options.triggerOnTouchLeave) {
					...
					//Trigger end handles as we swipe if thresholds met or if we have right the element if the user has asked to check these..
		 ...
		 * @param {int} bounds.right The rightmost value
	...
	_gsScope._gsDefine(
		...
		function (TweenPlugin, TweenLite) {
			...
			var _numExp = /(?:\-|\.|\b)(\d|\.|e\-)+/g,
				...
				_isSafariLT6, //Safari (and Android 4 which uses a flavor of Safari) has a bug that prevents changes to "top" and "right" properties from rendering properly if changed on the same frame as a transform UNLESS we set the element's WebkitBackfaceVisibility to hidden (weird, I know). Doing this for Android 3 and earlier seems to actually cause other problems, though (fun!)
				_ieVers,
				...
				_getComputedStyle = _doc.defaultView
					...
					: function () {},
				/**
				 * @private Returns the css style for a particular property of an element. For example, to get whatever the current "right" css value for an element with an ID of "myElement", you could do:
				 * var currentLeft = CSSPlugin.getStyle( document.getElementById("myElement"), "right");
				 *
				 * @param {!Object} t Target element whose style property you want to query
				 * @param {!string} p Property name (like "right" or "top" or "marginTop", etc.)
				 ...
				 * @param {!string} p Property name (like "right", "top", "marginRight", etc.)
				...
				_convertToPixels = (_internals.convertToPixels = function (
					...
					if (sfx === '%' && p.indexOf('border') !== -1) {
						...
						if (
							...
							style[horiz ? 'borderRightWidth' : 'borderTopWidth'] = v + sfx;
						}
				...
				_calculateOffset = (_internals.calculateOffset = function (t, p, cs) {
					//for figuring out "top" or "right" in px when it's "auto". We need to factor in margin with the offsetLeft/offsetTop
					if (_getStyle(t, 'position', cs) !== 'absolute') {
						return 0;
					}
					var dim = p === 'right' ? 'right' : 'Top',
				...
				// @private Parses position-related complex strings like "top right" or "50px 10px" or "70% 20%", etc. which are used for things like transformOrigin or backgroundPosition. Optionally decorates a supplied object (recObj) with the following properties: "ox" (offsetX), "oy" (offsetY), "oxp" (if true, "ox" is a percentage not a pixel value), and "oxy" (if true, "oy" is a percentage not a pixel value)
			 ...
			 * @param {boolean=} collapsible If true, the value is a top/left/right/bottom style one that acts like margin or padding, where if only one value is received, it's used for all 4; if 2 are received, the first is duplicated for 3rd (bottom) and the 2nd is duplicated for the 4th spot (right), etc.
			...
			var _getFormatter = function (dflt, clr, collapsible, multi) {
				 ...
				 * @param {!string} props a comma-delimited list of property names in order from top to right, like "marginTop,marginRight,marginBottom,marginLeft"
				 ...
				 * @param {!string} p Property that will be tweened (its name, like "right" or "backgroundColor" or "boxShadow")
			...
			var _transformProps =
				...
				_setIETransformRatio = function (v) {
					...
					if (!clip) {
						var mult = _ieVers < 8 ? 1 : -1, //in Internet Explorer 7 and before, the box model is broken, causing the browser to treat the width/height of the actual rotated filtered image as the width/height of the box itself, but Microsoft corrected that in IE8. We must use a negative offset in IE8 on the left/bottom
							marg,
							prop,
							dif;
						dx = t.ieOffsetX || 0;
			...
			_registerComplexSpecialProp('borderRadius', {
				...
				parser: function (t, e, p, cssp, pt, plugin) {
					...
					for (i = 0; i < props.length; i++) {
						...
						if (esfx !== bsfx) {
							hn = _convertToPixels(t, 'borderRight', bn, bsfx); //horizontal number (we use a bogus "borderRight" property just because the _convertToPixels() method searches for the keywords "Left", "Right", "Top", and "Bottom" to determine of it's a horizontal or vertical property, and we need "border" in the name so that it knows it should measure relative to the element itself, not its parent.
							vn = _convertToPixels(t, 'borderTop', bn, bsfx); //vertical number
							if (esfx === '%') {
								bs = (hn / w) * 100 + '%';
								bs2 = (vn / h) * 100 + '%';
							} else if (esfx === 'em') {
								em = _convertToPixels(t, 'borderRight', 1, 'em');
								bs = hn / em + 'em';
								bs2 = vn / em + 'em';
							} else {
								bs = hn + 'px';
			...
			p._onInitTween = function (target, vars, tween, index) {
				...
				if (_reqSafariFix)
					if (style.zIndex === '') {
						...
						if (v === 'auto' || v === '') {
							//corrects a bug in [non-Android] Safari that prevents it from repainting elements in their new positions if they don't have a zIndex set. We also can't just apply this inside _parseTransform() because anything that's moved in any way (like using "right" or "top" instead of transforms like "x" and "y") can be affected, so it is best to ensure that anything that's tweening has a z-index. Setting "WebkitPerspective" to a non-zero value worked too except that on iOS Safari things would flicker randomly. Plus zIndex is less memory-intensive.
				...
				if (vars.className) {
					//className tweens will combine any differences they find in the css with the vars that are passed in, so {className:"myClass", scale:0.5, right:20} would work.
				...
				if (this._transformType) {
					...
					if (!_transformProp) {
						...
						// 1) [non-Android] Safari skips rendering changes to "top" and "right" that are made on the same frame/render as a transform update.
				...
				for (p in vars) {
					...
					if (sp) {
						...
						if (
							...
							if ((bn || bn === 0) && (en || en === 0)) {
								//faster than isNaN(). Also, previously we required en !== bn but that doesn't really gain much performance and it prevents _parseToProxy() from working properly if beginning and ending values match but need to get tweened by an external plugin anyway. For example, a bezier tween where the target starts at right:0 and has these points: [{right:50},{right:0}] wouldn't work properly because when parsing the last point, it'd match the first (current) one and a non-tweening CSSPropTween would be recorded when we actually need a normal tween (type:0) so that things get updated during the tween properly.
								pt = new CSSPropTween(
									style,
									p,
									bn,
	...
	var _globals = window.GreenSockGlobals || window,
		...
		_split = function (element, vars, allChars, allWords, allLines) {
			...
			var text = _getText(element),
				...
				paddingRight = _getStyle(element, 'paddingRight', cs),
				borderTopAndBottom =
						...
						if (curLine[j].nodeName !== 'BR') {
							...
							if (absolute) {
								if (j === 0) {
									...
									lineNode.style.right = paddingRight + offset + 'px';
								}
								node.style.top = '0px';
								if (offset) {
									node.style.right = node._x - offset + 'px';
								}
							}
						}
					}
