	...
	var runSlider = function (container, opt) {
		...
		if (!container.hasClass('revslider-initialised')) {
			...
			container[0].addEventListener(
				'mouseleave',
				function () {
					container.trigger('tp-mouseright');
...
**************************************/
	...
	var checkHoverDependencies = function (_nc, opt) {
		...
		if (senter) {
			if (opt.layersonhover === undefined) {
				...
				opt.c.on('tp-mouseright', function () {
					if (opt.layersonhover)
						jQuery.each(opt.layersonhover, function (i, tnc) {
	...
	var contWidthManager = function (opt) {
		var rl = _R.getHorizontalOffset(opt.c, 'right');
		...
		if (
			...
			if (opt.sliderLayout == 'fullscreen' && opt.fullScreenAutoWidth == 'on')
				punchgs.TweenLite.set(opt.ul, { right: 0, width: opt.c.width() });
			else
		...
		if (
			...
			punchgs.TweenLite.set(opt.slayers, { right: rl });
	};
	...
	var prepareSlides = function (container, opt) {
		...
		if (opt.sliderType === 'carousel') {
			...
			opt.ul
				...
				.wrap(
					'<div class="tp-carousel-wrapper" style="width:100%;height:100%;position:absolute;top:0px;right:0px;overflow:hidden;"></div>'
				);
			var apt =
		...
		opt.allli.find('>img').each(function (j) {
			...
			img.wrap(
				'<div class="slotholder" style="position:absolute; top:0px; right:0px; z-index:0;width:100%;height:100%;"></div>'
			);
		...
		if (opt.scrolleffect.on && opt.scrolleffect.on_slidebg === 'on') {
			...
			opt.allli.find('.slotholder').each(function () {
				jQuery(this).wrap(
					'<div style="display:block;position:absolute;top:0px;right:0px;width:100%;height:100%" class="slotholder_fadeoutwrap"></div>'
				);
			});
	...
	var countDown = function (container, opt) {
		...
		container.on('tp-mouseright', function () {
