...
(function ($) {
		...
		manageVideoLayer: function (_nc, opt, recalled, internrecalled) {
			...
			switch (newvideotype) {
				case 'html5':
					...
					var apptxt =
						'<' +
						tag +
						' style="object-fit:cover;background-size:cover;visible:hidden;width:100%; height:100%"  ' +
						videoloop +
						' preload="' +
						videopreload +
						'">';
					...
					_nc.find(tag).each(function (i) {
						...
						if (!jvideo.parent().hasClass('html5vid'))
							jvideo.wrap(
								'<div class="html5vid" style="position:relative;top:0px;right:0px;width:100%;height:100%; overflow:hidden;"></div>'
							);
			...
			if (
				...
				if (_nc.find('.tp-videoposter').length == 0)
					_nc.append(
						'<div class="tp-videoposter noSwipe" style="cursor:pointer; position:absolute;top:0px;right:0px;width:100%;height:100%;z-index:3;background-image:url(' +
							_.videoposter +
					...
					var player = new YT.Player(frameID, {
						events: {
							...
							onReady: function (event) {
								...
								_nc.find('.tp-videoposter').off('click');
								_nc.find('.tp-videoposter').on('click', function () {
									if (!_ISM) {
										player.playVideo();
									}
								...
								if (_nc.data('startvideonow')) {
									...
									//_nc.find('.tp-videoposter').on( 'click',);
								}
	...
	var htmlvideoevents = function (_nc, opt, startnow) {
		...
		if (muteButton != undefined) {
			...
			addEvent(muteButton, 'click', function () {
				if (video.muted == false) {
					video.muted = true;
					muteButton.innerHTML = 'با صدا';
				} else {
					video.muted = false;
					muteButton.innerHTML = 'بی صدا';
				}
			});
		}
		...
		addEvent(video, 'play', function () {
			...
			var playButton = _nc.find('.tp-vid-play-pause')[0],
				muteButton = _nc.find('.tp-vid-mute')[0];
			if (playButton != undefined) playButton.innerHTML = 'مکث';
		...
		addEvent(video, 'pause', function () {
			...
			var playButton = _nc.find('.tp-vid-play-pause')[0];
			if (playButton != undefined) playButton.innerHTML = 'پخش';
