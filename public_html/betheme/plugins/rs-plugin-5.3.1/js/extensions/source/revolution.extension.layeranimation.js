		...
		prepareSingleCaption: function (obj) {
			...
			if (opt.debugMode) {
				_nc
					.closest('li')
					.find('.helpgrid')
					.css({ top: obj.offsety + 'px', right: obj.offsetx + 'px' });
		...
		createFrameOnTimeline: function (obj) {
			...
			tweens.content.eventCallback(
				...
				function (frame_index, frame_max, verylastframe, pw, _, tl, ust) {
					...
					opt.c.trigger('revolution.layeraction', [data]);
					if (data.eventtype !== 'rightstage') _R.animcompleted(_nc, opt);
					if (data.eventtype === 'rightstage')
...
**********************************************************************************************/
		...
		if (!nc.hasClass('tp-splitted')) {
			...
			if (_._nctype === 'column') {
				...
				if (_._cbgc_auto) {
					...
					if (jQuery.isArray(obj.marginRight)) {
	...
	var setColumnBgDimension = function (nc, opt) {
		...
		if (_._cbgc_man) {
			...
			if (!jQuery.isArray(_.cssobj.marginRight)) {
